...
export class MembrosFamiliasController {
    constructor(private readonly membrosFamiliasService: MembrosFamiliasService) {
    }
    ...
    create(@Body() createMembroFamiliaDto: CreateMembroFamiliaDto,
        ...
        return this.membrosFamiliasService.create(createMembroFamiliaDto, transaction);
    }
    ...
    @Patch(':id')
    update(@Param('id') id: string, @Body() updateMembroFamiliaDto: UpdateMembroFamiliaDto, @Req() request: CustomRequest
    ) {
        const transaction: Transaction = request.transaction;
        return this.membrosFamiliasService.update(+id, updateMembroFamiliaDto, transaction);
    }
